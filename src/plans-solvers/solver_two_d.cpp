...
namespace two_d
{
	namespace fourier
	{
		...
		template <class datatype>
		collocation_solver <datatype>::collocation_solver (plans::equation <datatype> &i_solver, utils::messenger* i_messenger_ptr, datatype& i_timestep, std::shared_ptr <bases::boundary <datatype>> i_boundary_0, std::shared_ptr <bases::boundary <datatype>> i_boundary_n) : 
		...
		template <class datatype>
		fourier_solver <datatype>::fourier_solver (plans::equation <datatype> &i_solver, datatype& i_timestep, std::shared_ptr <bases::boundary <datatype>> i_boundary_0, std::shared_ptr <bases::boundary <datatype>> i_boundary_n) : 
		...
		template <class datatype>
		laplace_solver <datatype>::laplace_solver (plans::equation <datatype> &i_solver, utils::messenger* i_messenger_ptr) : 
...
#pragma omp parallel for
		...
		template <class datatype>
		incompressible_corrector <datatype>::incompressible_corrector (plans::equation <datatype> &i_solver, plans::equation <datatype> &i_solver_x, plans::equation <datatype> &i_solver_z, utils::messenger* i_messenger_ptr) :
		...
		void incompressible_corrector <datatype>::factorize () {
			...
			utils::p_block_banded_factorize (id, np, m + (nbot == 0 ? 1 : -nbot - excess_n - 1) + (id == 0 ? 1: -excess_0 - ntop), 2, 1, &matrix [(id == 0 ? 0 : 1 + excess_0) * 6], &ipiv [0], &x [0], &xipiv [0], &bufferl [0], &bufferr [0], &info, ldn, 6, m + 2 + 3);
			// throw 0;
		}
