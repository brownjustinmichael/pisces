...
namespace plans
{
	namespace solvers
	{
		...
		private:
			...
			double gamma; //!< The ratio of the specific heats
			...
			double *data; //!< A pointer to the pressure data
			double *data_x; //!< A pointer to the x component of the data
			double *data_z; //!< A pointer to the z component of the data
			double *pressure; //!< A pointer to the pressure data
			double *density; //!< A pointer to the density data
			double *x_vel; //!< A pointer to the x component of the velocity
			double *z_vel; //!< A pointer to the z component of the velocity
			...
			double *new_pos; //!< A pointer to the halfway points along the grid, for convenience and speed
			...
			std::vector <double> matrix, positions, new_positions, bufferl, bufferr, buffer, data_temp; //!< The matrix data for the banded solve
			...
			std::vector <double> grad_pressure, grad_density;
			std::vector <double> grad2_pressure;

			grids::variable &rhs;

			std::shared_ptr <plan> transform;

			double *oodx, *oodx2, *oodz, *oodz2;
		...
		public:
			...
			private:
				...
				double *pressure; //!< The pointer to the pressure data
				double *density; //!< The pointer to the density data
			...
			public:
				 ...
				 ************************************************************************/
				factory (mpi::messenger *i_messenger_ptr, std::shared_ptr <boundaries::boundary> i_boundary_0, std::shared_ptr <boundaries::boundary> i_boundary_n, plans::solvers::equation &i_equation_x, plans::solvers::equation &i_equation_z, double *i_density, double *i_pressure, double i_gamma = 5. / 3.) : 
