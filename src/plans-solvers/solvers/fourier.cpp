...
namespace plans
{
	...
	fourier_solver <datatype>::fourier_solver (grids::grid <datatype> &i_grid_n, grids::grid <datatype> &i_grid_m, datatype& i_timestep, std::shared_ptr <plans::boundary <datatype>> i_boundary_0, std::shared_ptr <plans::boundary <datatype>> i_boundary_n, datatype *i_rhs, datatype* i_data, int *i_element_flags, int *i_component_flags) : 
	...
	void fourier_solver <datatype>::execute () {
		...
		*/
		...
		if (boundary_0) {
			boundary_0->calculate_rhs (data + excess_0, data, &data_temp [ex_overlap_0], &data_temp [ex_overlap_0 + excess_0], m, lda, x_solve);
		} else {
			linalg::matrix_add_scaled (1 + excess_0, ldn, 1.0, data, &data_temp [ex_overlap_0], m, lda);
		}
		if (boundary_n) {
			boundary_n->calculate_rhs (data + m - 1 - excess_n, data, &data_temp [ex_overlap_0], &data_temp [lda - 1 - excess_n - ex_overlap_n], m, lda, x_solve);
		} else {
