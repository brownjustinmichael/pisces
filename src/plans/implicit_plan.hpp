...
namespace plans
{
	...
	protected:
		...
		using plan::data_in;
		using plan::data_out;
		double *matrix_n; //!< A double pointer to the input data
		double *matrix_m; //!< A double pointer to the input data
	...
	public:
		 ...
		 ************************************************************************/
		implicit_plan (double *i_matrix_n, double *i_matrix_m, grids::variable &i_data_in, grids::variable &i_data_out, double i_coeff = 1.0, int state_in = spectral_spectral, int state_out = spectral_spectral) :
		plans::plan (i_data_in, i_data_out, state_in, state_out, i_coeff), 
		matrix_n (i_matrix_n), 
		matrix_m (i_matrix_m), 
