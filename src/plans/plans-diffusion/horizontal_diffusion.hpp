	...
	namespace diffusion
	{
		...
		public:
			...
			public:
				...
				virtual std::shared_ptr <plans::implicit_plan <datatype> > instance (grids::grid <datatype> **grids, datatype **matrices, datatype *i_data_in, datatype *i_data_out = NULL, int *i_element_flags = NULL, int *i_component_flags = NULL) const {
					...
					if (coeff) {
						return std::shared_ptr <plans::implicit_plan <datatype> > (new horizontal <datatype> (*grids [0], *grids [1], coeff, alpha, matrices [0], matrices [1], data_in, i_data_out, i_element_flags, i_component_flags));
					}
		...
		public:
			...
			public:
				 ...
				 ************************************************************************/
				virtual std::shared_ptr <plans::implicit_plan <datatype> > instance (grids::grid <datatype> **grids, datatype **matrices, datatype *i_data_in, datatype *i_data_out = NULL, int *i_element_flags = NULL, int *i_component_flags = NULL) const {
					return std::shared_ptr <plans::implicit_plan <datatype> > (new background_horizontal <datatype> (*grids [0], *grids [1], alpha, diffusion, matrices [0], matrices [1], i_data_in, i_data_out, i_element_flags, i_component_flags));
				}
			};
		};
	} /* diffusion */
