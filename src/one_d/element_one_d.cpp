...
namespace one_d
{
	namespace chebyshev
	{
		...
		element <datatype> (i_axis_n, i_name, params, i_messenger_ptr, i_flags) {
			...
			solvers [velocity]->add_pre_plan (new diffusion <datatype> (*grids [0], diffusion_coeff, alpha, matrix_ptr (velocity), ptr (velocity), ptr (vel_implicit_rhs)));
			if (advection_coeff != 0.0) {
				solvers [velocity]->add_post_plan (new advec <datatype> (*grids [0], advection_coeff, ptr (velocity), ptr (vel_explicit_rhs)));
			}
		...
		element <datatype> (i_axis_n, i_name, params, i_messenger_ptr, i_flags) {
			...
			if (params.nonlinear_diffusion_coeff != 0.0) {
				solvers [velocity]->add_post_plan (new nonlinear_diffusion <datatype> (*grids [0], params.nonlinear_diffusion_coeff, ptr (velocity), ptr (vel_explicit_rhs)));
			}
			if (advection_coeff != 0.0) {
				solvers [velocity]->add_post_plan (new advec <datatype> (*grids [0], advection_coeff, ptr (velocity), ptr (vel_explicit_rhs)));
			}
