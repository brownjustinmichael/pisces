...
namespace one_d
{
	...
	{
	public:
		master (int i_id, int i_p, std::string parameter_filename, int i_n_elements, int* n_grid, double* position_grid, std::string* name_grid, utils::messenger* i_messenger_ptr) : bases::master (i_id, i_p, i_n_elements, parameter_filename, i_messenger_ptr) {
			MTRACE ("Initializing...")
			for (int i = 0; i < i_n_elements; ++i) {
				MTRACE ("Adding element " << name_grid [i]);
				elements [i].reset (new Telement (n_grid [i] + 1, position_grid [i], position_grid [i + 1], name_grid [i], this->get_params (), i_messenger_ptr, 0x00));
				if (i != 0) {
