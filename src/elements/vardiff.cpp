		...
		for (YAML::const_iterator iter = i_params ["equations"].begin (); iter != i_params ["equations"].end (); ++iter) {
			...
			if (terms ["diffusion"].IsDefined ()) {
				...
				if (terms ["variable_diffusion"].IsDefined ()) {
					DEBUG ("Implementing...");
					equations [variable]->add_plan (typename diffusion::linear::factory (terms ["variable_diffusion.coefficient"].as <datatype> (), terms ["variable_diffusion.min"].IsDefined () ? terms ["variable_diffusion.min"].as <datatype> () : -(terms ["diffusion"].as <datatype> ()), (*this) [terms ["variable_diffusion.source"].as <std::string> ()], &diffusion [variable] [0], 100));
				}
