...
// 						for (int k = 0; k < kl; ++k) {
// 							for (int l = 0; l < k + 1; ++l) {
// 								x [i * ldxx + (j + ku) * ldx + k + kl + ku] -= bufferl [i * n * kl + j * n + l] * matrix [i * ldaa * lda + (l + k + ldaa - ku - 2 * kl) * lda + 2 * kl + ku - l];
// 							}
// 						}
// 					}
// 				}
...
// 						x [i * ldxx + (j + ku + kl) * ldx + ku + kl + k] = matrix [i * ldaa * lda + (j + ldaa - ku - kl) * lda + kl + ku + k - j];
// 					}
// 				}
// 				for (int j = 0; j < ku; ++j) {
// 					for (int k = -1; k >= -ku; --k) {
...
// 							x [i * ldxx + (j + ku + kl) * ldx + k + kl + ku] -= bufferr [i * n * ku + j * n + l] * matrix [i * ldaa * lda + (l + k + ldaa - ku - 2 * kl) * lda + 2 * kl + ku - l];
// 						}
// 					}
// 				}
// 				for (int k = 0; k < 2 * (ku + kl); ++k) {
...
//
// 				for (int i = 0; i < 2 * (kl + ku); ++i) {
// 					for (int j = 0; j < 2 * (kl + ku) * np; ++j) {
// 						debug << buffer [j * 2 * (kl + ku) + i] << " ";
// 					}
// 					DEBUG (debug.str ());
// 					debug.str ("");
// 				}
...
// 				}
//
// 				for (int j = 0; j < (ku + kl) * np; ++j) {
// 					for (int k = 0; k < (ku + kl) * np; ++k) {
// 						debug << x [q * ldxx + k * ldx + j] << " ";
// 					}
// 					DEBUG (debug.str ());
// 					debug.str ("");
// 				}
...
// 				for (int k = 0; k < kl; ++k) {
// 					for (int l = 0; l < kl - k; ++l) {
// 						y [i * ldy + k + 2 * kl + ku] -= b [i * ldb + l + ntop] * matrix [i * ldaa * lda + (l + k + ldaa - ku - 2 * kl) * lda + 2 * kl + ku - l];
// 					}
// 					debug << y [i * ldx + k + kl + ku] << " ";
// 				}
// 				DEBUG ("BEFORE GATHER " << debug.str ());
// 				debug.str ("");
// 			}
// 		}
// 		if (id == 0) {
...
// 				}
//
// 				for (int i = 0; i < (ku + kl) * np; ++i) {
// 					for (int k = 0; k < (ku + kl) * np; ++k) {
// 						debug << x [j * ldxx + k * ldx + i] << " ";
// 					}
// 					DEBUG ("F " << debug.str ());
// 					debug.str ("");
// 				}
//
// 				DEBUG ("Solving");
// 				matrix_solve ((ku + kl) * (np - 1), x + j * ldx * ldx + (kl + ku) * (ldx + 1), xipiv + j * ldx, &y [j * ldy + ku], info, 1, ldx);
// 				DEBUG ("Solved");
//
// 				for (int i = 0; i < ldy; ++i) {
// 					DEBUG ("Y DONE = " << y [j * ldy + i] << " " << j * ldy + i);
// 				}
//
// 				ycur = 0;
// 				bcur = 0;
...
// 		DEBUG ("OUT");
// 		for (int i = 0; i < nrhs; ++i) {
// 			matrix_copy (ntop, nrhs, &y [i * ldy], b + i * ldb, ntop + nbot, ldb);
// 			matrix_copy (nbot, nrhs, &y [ntop + i * ldy], b + ntop + n + i * ldb, ntop + nbot, ldb);
//
