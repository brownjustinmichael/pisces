...
__global__ void swap (int n, float* a, int lda, int k1, int k2, int* ipiv, int incx) {
	float temp;
	int ix;
	int ip;
	if (blockIdx.x == 0) {
...
__global__ void operation_left_upper (int m, int n, float* a, int lda, float* b, int ldb, bool nounit) {
	int j = blockIdx.x;
	while (j < n) {
		for (int k = m - 1; k >= 0; --k) {
			int i = threadIdx.x;
...
__global__ void operation_left_lower (int m, int n, float* a, int lda, float* b, int ldb, bool nounit) {
	int j = blockIdx.x;
	while (j < n) {
		for (int k = 0; k < m; ++k) {
			int i = threadIdx.x;
