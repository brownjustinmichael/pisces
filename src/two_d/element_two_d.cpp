	...
	namespace fourier
	{
		namespace chebyshev
		{
			...
			element <datatype> (i_axis_n, i_axis_m, i_name, params, i_messenger_ptr, i_element_flags) {
				...
				solvers [temp]->add_pre_plan (new vertical_diffusion <datatype> (*solvers [temp], params.diffusion_coeff, params.implicit_alpha));
				solvers [temp]->add_mid_plan (new horizontal_diffusion <datatype> (*solvers [temp], params.diffusion_coeff, params.implicit_alpha));
				if (params.advection_coeff != 0.0) {
					solvers [temp]->add_post_plan (new advection <datatype> (*solvers [temp], params.advection_coeff, ptr (x_vel), ptr (z_vel)));
				}
			...
			element <datatype> (i_axis_n, i_axis_m, i_name, params, i_messenger_ptr, i_element_flags) {
				...
				solvers [temp]->add_mid_plan (new horizontal_diffusion <datatype> (*solvers [temp], params.diffusion_coeff, params.implicit_alpha));
				if (params.advection_coeff != 0.0) {
					solvers [temp]->add_post_plan (new stream_advection <datatype> (*solvers [temp], params.advection_coeff, ptr (stream)));
				}
